// Threaded (reduce_sum) variant of hmm.stan. Identical model; the forward
// algorithm is moved verbatim from transformed parameters into
// partial_log_lik() and summed over households with reduce_sum, since
// households are independent given the parameters. Compile with
// stan_threads = TRUE and run with threads_per_chain (cmdstanr) /
// STAN_NUM_THREADS (rstan).
...
generated quantities {
  // Per-household log-likelihood, exposed as hmm.stan did via llik_final.
