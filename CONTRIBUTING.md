...
4. **Test locally.** Run `just test` or `just test-fast` before pushing.
5. **Open a pull request** against `main` with a clear description of
   what changed and why.

### Local Development Commands (`just`)

We use [`just`](https://github.com/casey/just) to automate common development workflows:
...
- R code is formatted with [`air`](https://github.com/posit-dev/air) and linted with `lintr` (configuration in `.lintr`).
- Do not hand-edit generated files: `R/stanmodels.R`, `R/globals.R`,
  ...
  `just docs` from roxygen comments in CI, so edit the roxygen, not the `.Rd`.
  `src/RcppExports.cpp` is generated by `Rcpp::compileAttributes()` and is tracked in git so direct GitHub installations (`devtools::install_github()`) succeed without requiring local code generation; synchronization is verified automatically by `just check-rcpp` and unit tests.
- See [AGENTS.md](AGENTS.md) for full developer and AI coding conventions.
...
| **lint** | Checks code formatting with `air` and runs `lintr::lint_package()`. |
